...
from crslab.model.crs.mhim.CapsuleNet import CapsuleNet
from crslab.model.crs.mhim.CapsuleNet import CapsuleLayer
from crslab.model.crs.mhim.ContextAwareFusion import ContextAwareFusion
from crslab.model.crs.mhim.fusion_module import PreferenceFusion, ContextGatedFusion
from crslab.model.crs.mhim.ContrastiveLoss import ContrastiveLoss
from crslab.model.crs.mhim.HyperGNN import HGNN
...
class MHIMModel(BaseModel):
    ...
    def _build_kg_layer(self):
        ...
        self.hyper_conv_session = HGNN(self.kg_emb_dim, self.kg_emb_dim)
        self.hyper_conv_knowledge = HGNN(self.kg_emb_dim, self.kg_emb_dim)
        # attention type
    ...
    def _attention_and_gating(self, session_embedding, knowledge_embedding, context_embedding, caps_embedding):
        related_embedding = torch.cat((session_embedding, knowledge_embedding, caps_embedding), dim=0)
    ...
    def _preference_fusion(self, session_embedding, knowledge_embedding, context_embedding, caps_embedding):
        pref_fusion = PreferenceFusion(emb_dim=128).to(self.device)
        context_fusion = ContextGatedFusion(emb_dim=128).to(self.device)
        fused = pref_fusion(session_embedding, knowledge_embedding, caps_embedding)  # 融合三种粒度偏好
        if context_embedding is not None:
            user_repr = context_fusion(fused, context_embedding)
        else:
            user_repr = fused
        return user_repr
    ...
    def encode_user(self, batch_related_entities, batch_related_items, batch_context_entities, kg_embedding):
        ...
        for session_related_entities, session_related_items, context_entities in zip(batch_related_entities,
            ...
            caps_input = session_embedding.to(self.device)  # [num_items, emb_dim]
            caps_embedding = capsule_layer(caps_input.unsqueeze(0)).squeeze(0)

            # session_embedding = torch.cat((session_embedding, caps_embedding), dim=0)
            ...
            # if len(context_entities) > 0:
            #     context_embedding = kg_embedding[context_entities].to(self.device)
            # user_repr = fusion_module(session_embedding,knowledge_embedding,context_embedding,caps_embedding).to(self.device)
            if len(context_entities) == 0:
                user_repr = self._preference_fusion(session_embedding, knowledge_embedding, None, caps_embedding)
            else:
                context_embedding = kg_embedding[context_entities]
                user_repr = self._preference_fusion(session_embedding, knowledge_embedding, context_embedding,
                                                    caps_embedding)
            user_repr_list.append(user_repr)
    ...
    def recommend(self, batch, mode):
        ...
        user_embedding = self.encode_user(
            related_entities, related_items, context_entities, kg_embedding
        )  # (batch_size, emb_dim)
    ...
    def decode_greedy(self, related_encoder_state, context_encoder_state, session_state, user_embedding):
        ...
        for i in range(self.longest_label):
            scores, incr_state = self.decoder(xs, related_encoder_state, context_encoder_state, session_state,
                                              incr_state)  # incr_state is always None
            scores = scores[:, -1:, :]
